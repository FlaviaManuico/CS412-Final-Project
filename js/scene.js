// scene.js
// Full 3D scene with textures, planets, moons, asteroids
// depends on createProgram/createSphere functions from planets.js
// depends on matrix helpers in transformations.js
...
// Planet data (texture paths should be relative to index.html)
const planetData = [
...
// Load texture, return WebGLTexture and store in textures map keyed by key (string)
function loadTexture(gl, key, url){
    ...
    image.onload = ()=>{
        ...
        // choose filtering: use mipmaps if power of two
...
function isPowerOf2(x){ return (x & (x - 1)) === 0 && x !== 0; }
...
function initScene(canvas){
    ...
    // create a shared sphere mesh (unit sphere)
    ...
    // Load textures: key them by planetName or planetName + '_' + moonName
    ...
    // Optionally load a Sun texture (if you have one)
...
function drawScene(){
    ...
    // helper to draw a sphere instance
    ...
    planetData.forEach(p=>{
        ...
        // draw planet (use planet name as texture key)
    ...
    // Spaceships placeholder orbiting Earth
    if(showSpaceships){
        const earth = planetData.find(p => p.name === "Earth");
        if(earth && earth.center){
            const sAngle = angle * 2;
            const sx = earth.center[0] + Math.cos(sAngle) * 2;
            const sz = earth.center[2] + Math.sin(sAngle) * 2;
            drawSphereInstance([sx,0,sz], 0.2, [1,0,0], false, null);
        }
    }
}

function resizeCanvas(canvas){
    // adapt size to CSS layout
